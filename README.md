...
graph TD
    ...
    subgraph Windows Service Backend (iltm.exe)
        C
        D
        E
        F
...
### 1. Servis Derleme
Proje kök dizininde aşağıdaki komut ile C++ backend derlemesini gerçekleştirin:
```powershell
...
### 2. Servis Kurulumu ve Başlatma
Servisi Windows Servis Kontrol Yöneticisi'ne (SCM) kaydetmek ve başlatmak için **Yönetici olarak açılmış PowerShell** üzerinde sırasıyla çalıştırın:

```powershell
# Servisi Kaydet
.\iltm.exe --install

# İlk Şifreleme Anahtarını (master.key) Üretmek İçin Konsol Modunda Başlatın
# Şifrenizi girin, bu işlem localde master.key oluşturacaktır.
.\iltm.exe

# Windows Servisini Başlatın
Start-Service ILTM_Secure_Service
```

### 3. Flutter Arayüzünü Çalıştırma
`gui` klasörü içerisine girerek kontrol panelini başlatın:
```powershell
cd gui
